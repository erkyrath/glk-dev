...
#   It can also generate the infglk.h header file (Inform 6).
#
# To run:   parse_dispatch.py --dispajs dispatch_dump.xml
#
# This will generate a bunch of lines of Javascript code -- the autogenerated
# proto_map section of gi_dispa.js. When the Glk API is updated, I'll need to
# re-run this and replace that block.
#
...
if (len(args) != 1):
    print 'usage: parse_dispatch.py dispatch_dump.xml'
    sys.exit(1)
...
class ArgChar(ArgBase):
    ...
    def tojs(self):
        if (self.signed is None):
            return 'new ArgChar(null)'
        elif (self.signed):
            return 'new ArgChar(true)'
        else:
            return 'new ArgChar(false)'
    def tolabel(self):
        if (self.signed is None):
            return 'nativechar'
        elif (self.signed):
...
class ArgInt(ArgBase):
    ...
    def tojs(self):
        if (self.signed):
            return 'new ArgInt(true)'
        else:
            return 'new ArgInt(false)'
    def tolabel(self):
        if (self.signed):
            return 'int'
        else:
...
class ArgClass(ArgBase):
    ...
    def tojs(self):
        return 'new ArgClass("%s")' % (self.name,)
    def tolabel(self):
        return self.name
