...
if not args:
    print 'Usage: i6squash source.inf'
    sys.exit(-1)
...
def show_state(state):
    bits = []
    for ix in range(len(bit_names)):
        if state & (1 << (ix+16)):
            bits.append(bit_names[ix])
    res = hex(state & 0xFFFF)
    if bits:
        res = res + ',' + ','.join(bits)
    return res

def scanner(state, ch):
    newcolor = None
    ...
    if state & SBIT_STATEMENT:
        ...
        if not state & SBIT_AFTERRESTART:
            return (None, state)
        (term, state) = innerstate(state, ch)
        newcolor = None
...
class ColorSpan:
    ...
    def __repr__(self):
        ...
        return '<%s%s: %s>' % (prefix, color_names[self.color], repr(self.text))
...
def parse(fl):
    ...
    while True:
        ...
        if ch == '':
            break
        (backtup, state) = scanner(state, ch)
        if state & SBIT_COLORBACKTRACK:
            ...
            ls = ls[ -backdist : ]
            if prels:
                spans.append(ColorSpan(''.join(prels), color, startline))
                startline = False
            color = backcol
        newcol = charcolor(state, ch)
        if newcol != color or ch == '\n':
            if ls:
                spans.append(ColorSpan(''.join(ls), color, startline))
                ls = []
                startline = False
            color = newcol
        if ch == '\n':
